...
Faça um git clone do projeto setup-node:
```
...
No projeto **setup-node** execute o comando abaixo:
```
yarn install
```
...
Dentro do projeto **setup-node** execute:
```
docker-compose up -d
```
...
p.s: Caso você esteja utilizando o docker ao invés de localhost utilize o ip que está definido no arquivo de docker-compose.yml do **setup-node** para realizar as consultas. ;-)